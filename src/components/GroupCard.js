...
export function renderGroupCard(group, teamCount, onSaveScore) {
  ...
  const rows = sorted.map((s, i) => {
    ...
    const diffStr = s.diff > 0
      ? `<span class="diff-pos">+${s.diff}</span>`
      : s.diff < 0
        ? `<span class="diff-neg">${s.diff}</span>`
    ...
    <tr class="${qcls}">
      ...
      <td class="c">${s.bp}</td>
      <td class="c">${s.bc}</td>
      <td class="c">${diffStr}</td>
      <td class="c"><strong class="pts-cell">${s.pts}</strong></td>
    </tr>`
  }).join('')

  const matchRows = group.matches.map(m => {
    const homeName = m.home?.name ?? m.home_name ?? '?'
    const awayName = m.away?.name ?? m.away_name ?? '?'
    const homeId = m.home?.id ?? m.home_player_id
    const awayId = m.away?.id ?? m.away_player_id

    return `
    <div class="match-row ${m.played ? 'match-played' : ''}">
      <div class="mt home">${homeName}</div>
      <div class="mscore">
        <input class="si" type="number" min="0" max="99"
          value="${m.score_home !== null && m.score_home !== undefined ? m.score_home : ''}"
          id="sh_${m.id}" placeholder="-">
        <span class="sep">–</span>
        <input class="si" type="number" min="0" max="99"
          value="${m.score_away !== null && m.score_away !== undefined ? m.score_away : ''}"
          id="sa_${m.id}" placeholder="-">
      </div>
      ...
      <button class="sv-btn"
        ...
        data-home-id="${homeId}"
        data-away-id="${awayId}"
        data-was-played="${m.played}"
        data-old-sh="${m.score_home ?? ''}"
        data-old-sa="${m.score_away ?? ''}">💾</button>
    </div>`
  }).join('')
  ...
  <div class="grp-card">
    ...
    <table class="stbl">
      <thead><tr>
        <th>#</th><th>Joueur</th>
        <th class="c" title="Matchs joués">J</th>
        <th class="c" title="Victoires">G</th>
        <th class="c" title="Nuls">N</th>
        <th class="c" title="Défaites">D</th>
        <th class="c" title="Buts pour">BP</th>
        <th class="c" title="Buts contre">BC</th>
        <th class="c" title="Différence">+/−</th>
        <th class="c" title="Points">Pts</th>
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>
...
export function renderBest3rds(best3) {
  ...
  <div class="best3-panel">
    <div class="best3-hd">⭐ <span>Meilleurs 3es — 4 qualifiés supplémentaires</span></div>
    <table class="stbl">
      <thead><tr>
        <th>#</th><th>Joueur</th><th>Groupe</th>
      ...
      <tbody>
        ${best3.map((s, i) => {
          const ac = ageCat(s.age)
          const d = s.diff > 0 ? `+${s.diff}` : s.diff
          return `<tr class="q3">
            <td><span class="rpip ${['r1','r2','r3','rn'][Math.min(i,3)]}">${i+1}</span></td>
            <td><div style="display:flex;align-items:center;gap:5px;">
              <div class="acat" style="background:${ac.color};"></div><strong>${s.name}</strong>
            </div></td>
