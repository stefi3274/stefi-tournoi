...
import { renderDrawModal, openDrawModal, closeDrawModal, attachDrawEvents, getDrawResult } from '../components/DrawModal.js'
...
export function renderAdminPage() {
  const nGroups = getGroupCount(teamCount)
  return `
  <div class="admin-grid">
    <div style="display:flex;flex-direction:column;gap:1.2rem;">
      ...
      <div class="card">
        ...
        <div class="card-bd">
          <p class="tmuted mb1">Ce logo s'affiche en en-tête de l'application.</p>
          <div style="display:flex;align-items:center;gap:12px;">
            ...
            <div>
              ...
              ${logoData ? `<button class="btn btn-d btn-sm" id="logo-reset" style="margin-left:6px;">✕ Supprimer</button>` : ''}
              <div class="tmuted" style="margin-top:6px;font-size:.72rem;">PNG · JPG · SVG · Format carré idéal</div>
            </div>
          </div>
        </div>
      </div>
  ...
  ${renderDrawModal(nGroups, players.length)}`
}
...
function renderTournoisList() {
  ...
  attachTournoisListEvents()
}

function attachTournoisListEvents() {
...
export function attachAdminEvents() {
  ...
  document.getElementById('logo-input')?.addEventListener('change', function() {
    ...
    reader.onload = e => {
      ...
      // Mettre à jour preview et header
      document.getElementById('logo-preview').innerHTML =
        `<img src="${logoData}" style="width:100%;height:100%;object-fit:cover;border-radius:12px;">`
      const hdrLogo = document.querySelector('.logo-icon, .logo-img')
      if (hdrLogo) {
        hdrLogo.outerHTML = `<img src="${logoData}" class="logo-img" alt="Logo">`
      }
...
function addPlayer() {
  ...
  if (!name) { showToast('Saisis le nom du joueur !', true); return }
